...
const navItems = [
  ...
  { href: "/chat", label: "Asistente IA", icon: "M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" },
  { href: "/analyzer", label: "Analizador de Config", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" },
...
export function Sidebar() {
  ...
  const [routerAlias, setRouterAlias] = useState("");
  ...
  useEffect(() => {
    ...
    fetch("/api/mikrotik")
      .then((res) => res.json())
      .then((json) => {
        if (json.alias) setRouterAlias(json.alias);
      })
      .catch(() => {});
  }, []);
    ...
    <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0">
      <div className="p-5 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-500 font-medium">Sentinel v3.0</p>
          </div>
        </div>
        {routerAlias && (
          <p className="text-xs text-slate-500 mt-2 truncate">Router: {routerAlias}</p>
        )}
      </div>
      ...
      <div className="p-4 border-t border-slate-800">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
          ...
          <span>
            {secStatus.active
              ? `Seguridad: ${secStatus.activeEvents} eventos` + (secStatus.altaCount > 0 ? ` (${secStatus.altaCount} alta)` : "")
              : "Seguridad: standby"}
          </span>
        </div>
        <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
          <div className={`w-2 h-2 rounded-full ${kbStats.totalChunks > 0 ? "bg-emerald-500" : "bg-slate-500"}`} />
          <span>{kbStats.totalChunks > 0 ? `BD: ${kbStats.totalChunks} fragmentos` : "BD: 19 estaticos"}</span>
        </div>
        <p className="text-xs text-slate-600 mt-1">
          {kbStats.lastSync
            ? `Docs: ${new Date(kbStats.lastSync).toLocaleDateString()}`
            : "Sincroniza docs desde el Panel"}
        </p>
      </div>
    </aside>
  );
