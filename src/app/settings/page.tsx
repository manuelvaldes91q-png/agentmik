...
export default function SettingsPage() {
  ...
  const handleFlushCache = async () => {
    if (!confirm("Esto eliminara TODOS los datos guardados: snapshots, eventos de seguridad, acciones pendientes, incidentes y patrones. Continuar?")) return;
    setStatus("saving");
    setStatusMsg("Limpiando cache...");
    ...
    try {
      ...
      if (data.success) {
        setStatus("success");
        setStatusMsg("Cache limpiada. Todos los datos anteriores eliminados.");
      } else {
        setStatus("error");
        setStatusMsg(data.error || "Error al limpiar cache.");
      }
    } catch {
      setStatus("error");
      setStatusMsg("Error de red al limpiar cache.");
    }
  };
      ...
      <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-6 space-y-5">
        ...
        <div className="flex gap-3 pt-2 flex-wrap">
          ...
          <button
            ...
            disabled={status === "testing" || status === "saving"}
            className="px-4 py-2.5 bg-orange-600 hover:bg-orange-500 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            Limpiar Cache (Reset Total)
          </button>
        </div>
      </div>
